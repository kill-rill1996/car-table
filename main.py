...
def main():
    ...
    skip_rows_count, errors = write_result_file(config["filename_from_1c"])

    # запись ошибок в errors.txt
    write_error_rows(skip_rows_count, errors)


def write_result_file(filename: str) -> (int, list):
    ...
    with open(filename, newline="\n", encoding="cp1251") as file:
        ...
        for row in reader:
            ...
            if len(row) < 34 or row[0] != "Товары":
                count += 1
                skip_row_count += 1
                errors.append([f"{count}"] + ["Строка некорректная"])
                continue

            count += 1

            # получаем готовую строку для записи
            result_row, row_errors = get_result_row(row)

            if row_errors:
                errors.append([f"{count}"] + row_errors)

            write_to_csv_file(result_row)

    return skip_row_count, errors


def get_result_row(csv_row: list) -> (list, list):
    ...
    product_info, error = get_product_types(csv_row[12], csv_row[13])
    if error:
    ...
    make_model_generation, error = get_make_model_generation(csv_row[15], csv_row[16], csv_row[17])
    if error:
        row_errors.append(error)
    ...
    return result_row, row_errors


def get_product_types(group_M: str, sub_group_N: str) -> (list, str):
    ...
    if not result:
        return ["ЗАГЛУШКА 1", "ЗАГЛУШКА 2", "ЗАГЛУШКА 3", "ЗАГЛУШКА 4", "ЗАГЛУШКА 5"], \
            f"Не удалось найти соответствие ProductType, SparePartType, EngineSparePartType, BodySparePartType, " \
            f"DeviceType по группе '{group_M}' и подгруппе '{sub_group_N}'"
...
def get_make_model_generation(make: str, model: str, modification: str) -> (list, str):
    ...
    for row in MAKES_MODELS_GENERATIONS:
        if row[10] == make and row[11] == model and row[12] == modification:
            return row, ""

    return ["ЗАГЛУШКА 1", "ЗАГЛУШКА 2", "ЗАГЛУШКА 3", "ЗАГЛУШКА 4", "ЗАГЛУШКА 5", "ЗАГЛУШКА 6", "ЗАГЛУШКА 7",
            "ЗАГЛУШКА 8", "ЗАГЛУШКА 9"], \
        f"Не удалось найти соответствие Make, Model, Generation, Modification, FuelType, DriveType, " \
...
def write_error_rows(skip_rows_count: int, errors: list):
    """Записывает необработанные строки из файла выгрузки 1С"""
    with open("errors.txt", "w") as file:
        file.write("СПИСОК ОШИБОК\n\n")

    if len(errors) > 0:
        text = f"Пропущено строк: {skip_rows_count}, всего ошибок: {len(errors)}\n\n" \
               f"Ошибки в строках по номерам в файле выгрузки из 1С:\n\n"

        for error in errors:
            text += f"{error[0]}. {' | '.join(error[1:])}\n"

        with open("errors.txt", "a") as file:
            file.write(text)
        print(f"Пропущено строк: {skip_rows_count}, всего ошибок {len(errors)}, они записаны в файле errors.txt")
    else:
        with open("errors.txt", "a") as file:
